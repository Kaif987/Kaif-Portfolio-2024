...
export default function ProjectCard({
  ...
  return (
    <div className="relative card rounded-lg overflow-hidden w-96 sm:w-80 h-80 px-3">
      <Image
        src={image_url}
        alt="project image"
        width={0}
      ...
      />
      <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-white bg-slate-900 opacity-80 px-4 py-2 rounded text-2xl text-center font-bold ">
        {name}
      </span>
      <div className="description">
        <div className="absolute inset-0 bg-sky-950 px-5 text-center flex flex-col justify-center items-center">
          ...
          <div className="flex justify-center items-start gap-3 text-white mt-8">
            {languages.map((language) => {
              return (
                <span key={language} className="bg-gray-600 px-2 py-1 rounded">
                  {language}
                </span>
              );
            })}
